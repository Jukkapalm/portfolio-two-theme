...
function haeMerkinnat(): void {
    ...
    try {
        ...
        $lause = $yhteys->prepare("SELECT id, created_at, merkinta FROM blogi_merkinnat ORDER BY created_at DESC");
        ...
        // Suojataan HTML erikoismerkeiltä
...
function tallennaMerkinta(): void {
    ...
    $pinOikein = isset($data["pin"]) && hash_equals(BLOGI_PIN, (string)$data["pin"]);

    if (!$pinOikein) {
        http_response_code(401);
        echo json_encode(["virhe" => "Väärä PIN-koodi"]);
        return;
    }
    ...
    try {
        ...
        $lause = $yhteys->prepare("INSERT INTO blogi_merkinnat (merkinta) VALUES (:merkinta)");
        ...
        echo json_encode(["ok" => true, "viesti" => "Merkinta tallennettu"]);
    } catch (PDOException $e) {
